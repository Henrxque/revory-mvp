...
export default async function FindingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const finding = await getQuoteRecoveryFindingDetail(context.workspace.id, id);
  if (!finding) notFound();
  const evidence = Array.isArray(finding.evidenceJson)
    ? finding.evidenceJson as Array<{ field: string; value: string | number | null; provenance?: { fileName?: string; rowNumber?: number } }>
    : [];

  return (
    <div className="space-y-6">
      <Link className="text-sm font-semibold text-[color:var(--accent-light)]" href="/app/revenue-leaks">← Back to opportunities</Link>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="rev-shell-panel rounded-[26px] p-5">
          <p className="rev-label">Source evidence</p>
          <h2 className="mt-2 text-xl font-bold">Why REVORY flagged this estimate</h2>
        ...
        <aside className="rev-shell-panel rounded-[26px] p-5">
          <p className="rev-label">Recommended review</p>
          <p className="mt-3 text-sm leading-6 text-[color:var(--text-muted)]">{finding.recommendedAction}</p>
          <div className="mt-5 grid gap-2">
            <form action={acknowledgeQuoteRecoveryFinding.bind(null, finding.id)}><button className="rev-button-secondary w-full">Acknowledge</button></form>
            <form action={resolveQuoteRecoveryFinding.bind(null, finding.id)}><button className="rev-button-primary w-full">Mark reviewed</button></form>
            <form action={recoverQuoteRecoveryFinding.bind(null, finding.id)} className="rounded-2xl border border-[color:var(--border-accent)] p-3">
              <label className="text-xs font-bold" htmlFor="recoveredValue">Confirmed recovered value ({finding.currency})</label>
              <input className="rev-input-field mt-2 w-full" id="recoveredValue" min="0" name="recoveredValue" placeholder="0" step="0.01" type="number" />
              <button className="rev-button-primary mt-2 w-full">Mark recovered</button>
            </form>
            <form action={dismissQuoteRecoveryFinding.bind(null, finding.id)}><button className="rev-button-secondary w-full">Dismiss false positive</button></form>
          </div>
          <p className="mt-4 text-xs leading-5 text-[color:var(--text-subtle)]">Recovered value is customer-confirmed and remains separate from the original estimated opportunity.</p>
        </aside>
      </section>
    </div>
  );
