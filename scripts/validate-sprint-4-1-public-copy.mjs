...
for(const pattern of futureTerms){const match=landing.match(pattern);if(match){const window=landing.slice(Math.max(0,(match.index??0)-260),(match.index??0)+360);if(!/roadmap|gated|not sold|require Sprints/i.test(window))throw new Error(`Future claim ${pattern} is not visibly gated`)}}
