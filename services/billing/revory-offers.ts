...
import { isStripeBillingConfigured, isStripeWebhookConfigured } from "@/services/billing/stripe-runtime";
...
export function revoryStripePriceMatchesContract(
  key: RevoryOfferKey,
  price: { active?: boolean; currency?: string; id?: string; recurring?: { interval?: string; interval_count?: number } | null; unit_amount?: number | null },
) {
  ...
  return price.active !== false
    ...
    && (offer.mode === "payment" ? price.recurring == null : price.recurring?.interval === offer.interval && price.recurring.interval_count === 1);
}
