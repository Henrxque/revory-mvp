...
export async function enforceWorkspaceRetention(
  ...
  if (!settings) {
    return { deletedFindings: 0, deletedImportSessions: 0, deletedRuns: 0, skipped: true };
  }
  const cutoff = new Date(now.getTime() - settings.retentionDays * 24 * 60 * 60 * 1000);
  return prisma.$transaction(async (tx) => {
    const [findings, runs, sessions] = await Promise.all([
    ...
    const deletedCount = findings.count + runs.count + sessions.count;
    if (deletedCount > 0) {
