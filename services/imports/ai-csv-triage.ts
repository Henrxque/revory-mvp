...
export async function requestAiCsvTriage(
  ...
  const aiOutput = await requestBoundedStructuredOutput({
    ...
    prompt:
      "Review this sanitized legacy CSV profile only as a bounded migration-compatibility assistant for REVORY. The active product is Revenue Leak Intelligence for high-ticket service businesses; the appointment/client fields in this request belong to the discontinued MedSpa substrate and must never be translated into contractor estimate behavior or presented as current product evidence. Return only the strict JSON schema. Suggest only mappings supported by the supplied legacy canonical fields, but never import data, create revenue leaks, calculate revenue, claim confirmed loss, or override deterministic validation. Every mapping remains subject to user review. The sample rows contain shape labels rather than raw personal data. Do not reconstruct or request names, emails, phones, notes, medical details, documents, or full CSV content. PAYMENTS_UNSUPPORTED and UNKNOWN must remain honest. reviewRequired must always be true.",
    schema,
