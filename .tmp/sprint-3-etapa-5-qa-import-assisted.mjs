...
async function waitForOnboardingStepProgress(expectedStep, timeoutMs = 45000) {
  const startedAtMs = Date.now();

  while (Date.now() - startedAtMs < timeoutMs) {
    const snapshot = await getWorkspaceSnapshot();

    if (snapshot?.activationSetup?.currentStep === expectedStep) {
      return snapshot;
    }
...
async function waitForStepPage(page, step) {
  const ensureStepSurface = async () => {
    switch (step) {
      case "template":
        await page.locator('input[value="MEDSPA"]').waitFor({
          state: "visible",
          timeout: 60000,
        });
        break;
      case "source":
        await page.locator('input[value="APPOINTMENTS_CSV"]').waitFor({
          state: "visible",
          timeout: 60000,
        });
        break;
      case "channel":
        await page.locator('input[value="EMAIL"]').waitFor({
          state: "visible",
          timeout: 60000,
        });
        break;
      case "reviews":
        ...
        break;
      case "mode":
        await page.locator('input[value="MODE_B"]').waitFor({
          state: "visible",
          timeout: 60000,
        });
        break;
  ...
  if (!page.url().includes(`/app/setup/${step}`)) {
    ...
    page.url().includes(`/app/setup/${step}`),
    `Expected onboarding step ${step}, but current URL is ${page.url()}.`,
  );
  await ensureStepSurface();
}
