...
- Sprint 16 operational-control preparation is implemented with a GitHub-hosted external health workflow on the default branch, a redacted cron observer and a guarded isolated-restore verifier; the isolated restore passed on 2026-07-19, while the first controlled alert dispatch was blocked by a GitHub hosted-runner `startup_failure`, and observed cron runs, MFA ownership and Stripe test-mode lifecycle proof are still missing, so the Sprint 16 exit remains blocked;
